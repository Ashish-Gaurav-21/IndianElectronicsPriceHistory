...
def extract_data(pagination_url):
    ...
    for product in soup.select('ul.products li.product'):
        product_dict = {}
        product_dict['product_id'] = product.select('.add_to_cart_button')[0].get('data-product_id', 'n/a') if product.select('.add_to_cart_button') else 'n/a'
        product_dict['model_no'] = product.select('.add_to_cart_button')[0].get('data-product_sku', 'n/a') if product.select('.add_to_cart_button') else 'n/a'
...
pagination_url = 'https://smcinternational.in/product-category/gaming-keyboard/'
# pagination_url = 'https://smcinternational.in/product-category/cable/'
