...
def extract_data(pagination_url):
    ...
    for current_page in range(1, int(num_pages) + 1):
        for product in soup.select('.main-products-wrapper .product-thumb'):
            ...
            product_dict['model_no'] = ' '.join(product.select('.stats-label')[0].find_all(text=True)[-1].split()) if product.select('.stats-label') else 'n/a'
            ...
            product_dict['review_count'] = 'n/a' # Not displayed on PL. But, available in PDP.
            ...
            product_dict['stock_status'] = [i for i in product.select('.product-labels')[0].find_all(text=True) if 'stock' in i.lower()] if product.select('.product-labels') else 'n/a'
            product_dict['stock_status'] = product_dict['stock_status'][0] if isinstance(product_dict['stock_status'], list) and (len(product_dict['stock_status']) > 0) else 'In Stock'
            if product_dict['product_id'] != 'n/a': data_list.append(product_dict)
        soup = get_soup(pagination_url, str(current_page + 1))
    return data_list

    
# pagination_url = 'https://www.vedantcomputers.com/pc-components/processor'
pagination_url = 'https://www.vedantcomputers.com/pc-components/graphics-card'
