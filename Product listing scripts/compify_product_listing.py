...
def get_soup(pagination_url, page_num=1):
    ...
    '''Get soup object for the Product listing url. Default page number of 1 will be used. Site displays maximum of 16 products on a single page.'''
...
def extract_data(pagination_url):
    ...
    num_pages = int(ceil(int(result_count) / 16)) if result_count != 'n/a' else 'n/a'
    data_list = []
    for current_page in range(1, int(num_pages) + 1):
        for product in soup.select('#shop-product-wrap li.thunk-woo-product-list'):
            product_dict = {}
            product_dict['product_id'] = product.select('a.button.product_type_simple')[0].get('data-product_id', 'n/a') if product.select('a.button.product_type_simple') else 'n/a'
            product_dict['model_no'] = product.select('a.button.product_type_simple')[0].get('data-product_sku', 'n/a') if product.select('a.button.product_type_simple') else 'n/a'
            product_dict['product_page_url'] = product.select('a.button.product_type_simple')[0].get('href', 'n/a') if product.select('a.button.product_type_simple') else 'n/a'
            product_dict['product_name'] = ' '.join(product.select('h2.woocommerce-loop-product__title')[0].text.split()) if product.select('h2.woocommerce-loop-product__title') else 'n/a'
            ...
            product_dict['stock_status'] = 'Out of Stock' if 'outofstock' in product.attrs.get('class', ['n/a']) else 'In Stock'
...
pagination_url = 'https://compify.in/product-category/desktop-processors/'
# pagination_url = 'https://compify.in/product-category/memory-ram/'
# pagination_url = 'https://compify.in/product-category/graphics-cards/'
