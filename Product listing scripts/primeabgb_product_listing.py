...
def extract_data(pagination_url):
    ...
    '''Extract the data of product present in listing page. product_id, model_no, product_name, price, brand, url, stock status, average_rating.'''
    ...
    for current_page in range(1, int(num_pages) + 1):
        for product in soup.select('ul.products li.product-item'):
            product_dict = {}
            product_dict['product_id'] = product.select('a.product_type_simple')[0].get('data-product_id', 'n/a') if product.select('a.product_type_simple') else 'n/a'
            product_dict['model_no'] = product.select('a.product_type_simple')[0].get('data-product_sku', 'n/a') if product.select('a.product_type_simple') else 'n/a'
            ...
            product_dict['stock_status'] = 'Out of Stock' if product.select('.out-of-stock') else 'In Stock'
            product_dict['average_rating'] = product.select('strong.rating')[0].text.strip() if product.select('strong.rating') else 'n/a'
            brand = [i.replace('pa_brand-', '') for i in product.get('class', ['n/a']) if 'pa_brand-' in i]; product_dict['brand'] = brand[0] if isinstance(brand, list) and (len(brand) > 0) else 'n/a'
            if product_dict: data_list.append(product_dict)
        soup = get_soup(pagination_url, str(current_page + 1))
    return data_list


pagination_url = 'https://www.primeabgb.com/buy-online-price-india/cpu-processor/'
# pagination_url = 'https://www.primeabgb.com/buy-online-price-india/graphic-cards-gpu/'
